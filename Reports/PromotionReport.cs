...
namespace SlotPOS
{
    public partial class PromotionReport : Form
    {
        ...
        private void FillCombo()
        {
            ...
            var getPromotionCat = (from a in ctx.tblItemCategories
                                   where a.Promotions ==true
                               select a).ToList();
            getPromotionCat.Insert(0, new tblItemCategory { ItemCategoryID = 0, ItemCategory = "-- All --" });
            if (getPromotionCat.ToList().Count > 0)
            {
                cmbCategory.DataSource = getPromotionCat;
        ...
        private void BindReport()
        {
            ...
            paramList.Add(new ReportParameter("UserID", comboBox1.Text, true));
            paramList.Add(new ReportParameter("CategoryID", cmbCategory.Text, true));
            ...
            SlotDataSetTableAdapters.sp_PromotionTableAdapter ad_mis = new SlotDataSetTableAdapters.sp_PromotionTableAdapter();
           

            DataSet ds = new DataSet();

            ds.Tables.Add(ad_mis.GetData(dtToDate.Value, dtFromDate.Value, long.Parse(comboBox1.SelectedValue.ToString()), long.Parse(cmbCategory.SelectedValue.ToString())));
            ...
            using (StreamReader rdlcSR = new StreamReader(@"RdlcReport\rptPromotion.rdlc"))
            {
